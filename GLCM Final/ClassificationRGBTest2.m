...
cmHSVchannel = 2; % HSV channel to use (Hue 1, Saturation 2, Value 3)
cmGF = 2; % Apply Gaussian blur to sample and scene
cmGL = 6; % Gray levels 
cmDist = 2;
cmDir = [1 1; -1 1];
...
classWeight = 0.25; % Classification bias (0 Field always wins, 1 Tree always wins)
%Field
...
% Field
samplePropsField = NaN(size(cmProps, 2), size(cmDir, 1), sampleCasesField, 3);
for sf = 1:sampleFilesField

    sampleImg = readimage(sampleImdsField, sf);
    
    sampleImg2 = sampleImg;
    %if cmHSV == false
    ...
    for rot = 1:sampleRot
        ...
        samplePropsField(:, :, (sf-1)*sampleRot+rot, cc) = extractFeaturesGLCM(sampleImg2Rot(:,:,cc), cmDir*cmDist, cmSym, cmProps, cmGL);
        end
    end
end
...
for sf = 1:sampleFilesTree
    ...
    for rot = 1:sampleRot
        for cc = 1:3
        sampleImg2Rot = rot90(sampleImg2, rot-1);
        samplePropsTree(:, :, (sf-1)*sampleRot+rot, cc) = extractFeaturesGLCM(sampleImg2Rot(:,:,cc), cmDir*cmDist, cmSym, cmProps, cmGL);
        end
    end
end
...
parfor ny = 1:swNumY
    for nx = 1:swNumX
        ...
        swPropsRGB = NaN(size(cmProps, 2), size(cmDir, 1), 3);
        for cc = 1:3
            swPropsRGB(:,:,cc) = extractFeaturesGLCM(swSample(:,:,cc), cmDir*cmDist, cmSym, cmProps, cmGL);
            swProps = mean(swPropsRGB, 3);
        end

        for sc = 1:sampleCasesField, errField(ny, nx, sc) = sum( abs(samplePropsField(:,:,sc) - swProps), 'all'); end
        for sc = 1:sampleCasesTree, errTree(ny, nx, sc) = sum( abs(samplePropsTree(:,:,sc) - swProps), 'all'); end
